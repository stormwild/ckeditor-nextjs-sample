...
import CustomEditor from '../src/ckeditor'
...
describe('CustomEditor build', () => {
  ...
  describe('build', () => {
    it('contains plugins', () => {
      expect(CustomEditor.builtinPlugins).to.not.be.empty
    })

    it('contains config', () => {
      expect(CustomEditor.defaultConfig.toolbar).to.not.be.empty
    })
  })

  describe('create()', () => {
    beforeEach(() => {
      return CustomEditor.create(editorElement).then((newEditor) => {
        editor = newEditor
      })
    })
  ...
  describe('config', () => {
    ...
    it('allows configuring toolbar items through config.toolbar', () => {
      return CustomEditor.create(editorElement, {
        toolbar: ['bold'],
      }).then((newEditor) => {
        editor = newEditor
    ...
    it('allows removing built-in toolbar items', () => {
      return CustomEditor.create(editorElement, {
        ...
        expect(
          editor.ui.view.toolbar.items.find((item) => item.label === 'Italic')
        ).to.be.undefined
      })
    })
  })
  ...
  describeMemoryUsage(() => {
    testMemoryUsage('should not grow on multiple create/destroy', () =>
      CustomEditor.create(document.querySelector('#mem-editor'))
    )
  })
})
